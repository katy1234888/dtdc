...
with st.sidebar:
    ...
    st.html('<hr class="sidebar-divider">')
    st.html('<div style="font-size:10px;color:#8a93a8;letter-spacing:1px;">Q4 Festive Surge Analysis · FY 2025</div>')
...
if page == "🏠  Home":
    ...
    # Context
    st.html('<div class="section-heading">Case Background</div>')
...
# ═════════════════════════════════════════════
    ...
    st.html('<div class="section-heading">Upload Datasets</div>')
    st.html('<div class="section-sub">Import your 6 CSV files: Orders, NPS, Hub Performance, Courier Performance, Customers, Complaints</div>')
    ...
    if uploaded_files:
        ...
        for name, df in st.session_state['datasets'].items():
            with st.expander(f"  {name}  —  {df.shape[0]:,} rows × {df.shape[1]} columns"):
                ...
                with c1:
                    st.html('<div style="font-size:11px;letter-spacing:1px;color:#8a93a8;text-transform:uppercase;margin-bottom:6px;">Preview</div>')
                    st.dataframe(df.head(), use_container_width=True)
                with c2:
                    st.html('<div style="font-size:11px;letter-spacing:1px;color:#8a93a8;text-transform:uppercase;margin-bottom:6px;">Missing Values</div>')
...
# ═════════════════════════════════════════════
    ...
    st.html('<div class="section-heading">ML Data Imputation</div>')
    st.html('<div class="section-sub">MICE — Multivariate Imputation by Chained Equations</div>')
    ...
    if not st.session_state['datasets']:
        ...
        st.html('<div class="section-heading" style="font-size:18px;">Post-Cleaning Summary</div>')
        for name, df in st.session_state['datasets'].items():
...
# ═════════════════════════════════════════════
    ...
    st.html('<div class="section-heading">Command Center</div>')
    st.html('<div class="section-sub">Real-time operational KPIs · Q4 Festive Surge</div>')
    ...
    if not st.session_state['datasets']:
        ...
        with col_left:
            st.html('<div class="section-heading" style="font-size:18px;">Operational Throughput by Hub</div>')
            if hubs is not None:
                ...
                st.html('<div class="content-card" style="height:280px;display:flex;align-items:center;justify-content:center;color:#8a93a8;">Upload hub_performance.csv</div>')
        ...
        with col_right:
            st.html('<div class="section-heading" style="font-size:18px;">RTO Distribution by City</div>')
            if hubs is not None:
        ...
        # Charts row 2
        st.html('<div class="section-heading">Customer Sentiment & Issue Distribution</div>')
        ...
        with col_s1:
            if nps is not None:
                ...
                st.html('<div class="content-card" style="height:280px;display:flex;align-items:center;justify-content:center;color:#8a93a8;">Upload nps.csv</div>')
        ...
        with col_s2:
            if complaints is not None:
                ...
                st.html('<div class="content-card" style="height:280px;display:flex;align-items:center;justify-content:center;color:#8a93a8;">Upload complaints.csv</div>')

        st.html('<div class="info-box success">✓ Dashboard synchronized with Q4 Festive Surge metrics.</div>')
...
# ═════════════════════════════════════════════
    ...
    st.html('<div class="section-heading">End-to-End Funnel Analysis</div>')
    st.html('<div class="section-sub">Service erosion from order placement to detractor conversion</div>')
    ...
    if not st.session_state['datasets']:
        ...
        with col_m1:
            ...
            if orders is not None and complaints is not None:
                try:
                    ...
                    st.html(f'<div style="font-family:Barlow Condensed,sans-serif;font-size:48px;font-weight:800;color:#d0222a;">{ratio:.1f}%</div>')
                except:
                    st.html('<div style="color:#8a93a8;">Data unavailable</div>')
            st.html('<div style="font-size:12px;color:#8a93a8;margin-top:8px;">High correlation confirms delays as primary ticket driver</div></div>')
        ...
        with col_m2:
            ...
            if complaints is not None and nps is not None:
                try:
                    ...
                    st.html(f'<div style="font-family:Barlow Condensed,sans-serif;font-size:48px;font-weight:800;color:#fbbf24;">{ratio_det:.1f}%</div>')
                except:
                    st.html('<div style="color:#8a93a8;">Data unavailable</div>')
            st.html('<div style="font-size:12px;color:#8a93a8;margin-top:8px;">Resolution efficiency directly impacts loyalty</div></div>')
        ...
        with col_m3:
            ...
            if customers is not None:
                try:
                    ...
                    st.html(f'<div style="font-family:Barlow Condensed,sans-serif;font-size:48px;font-weight:800;color:#4ade80;">{repeat_rate:.1f}%</div>')
                except:
                    st.html('<div style="color:#8a93a8;">Data unavailable</div>')
            st.html('<div style="font-size:12px;color:#8a93a8;margin-top:8px;">Current loyalty baseline</div></div>')

        st.html("<hr>")

        # Root causes
        st.html('<div class="section-heading">Root Cause Analysis</div>')
        ...
        # Recommendations
        st.html('<div class="section-heading">Business Recommendations</div>')
        ...
        with rec_l:
            ...
            _html_l = """<div class="content-card"><div style="font-family:'Barlow Condensed',sans-serif;font-size:20px;font-weight:700;color:#4ade80;margin-bottom:16px;letter-spacing:1px;">⚡ QUICK WINS — SHORT TERM</div>"""
            for item in _items_l:
                _html_l += f"""<div style="display:flex;gap:12px;margin-bottom:12px;align-items:flex-start;"><div style="width:6px;height:6px;background:#d0222a;border-radius:50%;margin-top:6px;flex-shrink:0;"></div><div style="font-size:13px;color:#b0b8cc;line-height:1.5;">{item}</div></div>"""
            _html_l += "</div>"
            st.html(_html_l)
        ...
        with rec_r:
            ...
            _html_r = """<div class="content-card"><div style="font-family:'Barlow Condensed',sans-serif;font-size:20px;font-weight:700;color:#38bdf8;margin-bottom:16px;letter-spacing:1px;">🏗 STRATEGIC ROADMAP — LONG TERM</div>"""
            for item in _items_r:
                _html_r += f"""<div style="display:flex;gap:12px;margin-bottom:12px;align-items:flex-start;"><div style="width:6px;height:6px;background:#4361ee;border-radius:50%;margin-top:6px;flex-shrink:0;"></div><div style="font-size:13px;color:#b0b8cc;line-height:1.5;">{item}</div></div>"""
            _html_r += "</div>"
            st.html(_html_r)
        ...
        # KPI table
        st.html('<div class="section-heading">2026 Monitoring Framework</div>')
        st.markdown("""
        <div class="content-card">
